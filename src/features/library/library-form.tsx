...
import { WorkOutcomeFields } from "@/features/works/work-outcome-fields";
...
export function LibraryForm({ kind, item, tags = [], selectedTagIds = [], designVersions = [], assets = [], selectedAssetIds = [], workOutcomeReady = true }: { kind: LibraryKind; item?: LibraryRecord; tags?: Tag[]; selectedTagIds?: string[]; designVersions?: DesignVersionChoice[]; assets?: WorkAssetChoice[]; selectedAssetIds?: string[]; workOutcomeReady?: boolean }) {
  ...
  })}</div>{definition.supportsTags && <TagSelector initialTags={tags} initialSelectedIds={selectedTagIds}/>} {kind === "works" && workOutcomeReady && <WorkOutcomeFields item={item} designVersions={designVersions} assets={assets} initialAssetIds={selectedAssetIds}/>} {kind === "works" && !workOutcomeReady && <p role="status" className="rounded-xl border border-amber-800 bg-amber-950/30 px-4 py-3 text-sm text-amber-200">来源设计、实际材料与制作反馈暂时无法加载；保存文字资料不会清除这些已有关系。</p>} {state.error && <p role="alert" className="rounded-xl border border-red-900/50 bg-red-950/30 px-4 py-3 text-sm text-red-300">{state.error}</p>}<div className="flex flex-col-reverse gap-3 border-t border-border pt-6 sm:flex-row sm:justify-end"><Link href={cancelHref} className={buttonStyles({ variant: "secondary", className: "h-12" })}>取消</Link><SubmitButton label={draft ? "完成" : item ? "保存修改" : `添加${definition.singular}`} /></div></form>;
}
