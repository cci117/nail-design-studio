...
export function LibraryForm({ kind, item, tags = [], selectedTagIds = [] }: { kind: LibraryKind; item?: LibraryRecord; tags?: Tag[]; selectedTagIds?: string[] }) {
  ...
  const cancelHref = item ? `${definition.path}/${item.id}` : definition.path;
  return <form action={formAction} className="space-y-5"><div className="grid gap-5 sm:grid-cols-2">{definition.fields.map((field) => {
    const shared = { id: field.name, name: field.name, required: field.required, defaultValue: displayValue(item, field.name), placeholder: field.placeholder, className: "mt-2 w-full rounded-xl border border-border bg-surface px-4 text-sm text-white placeholder:text-zinc-700 focus:border-zinc-500 focus:outline-none" };
